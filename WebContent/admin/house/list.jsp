    ...
    $.getJSON("../../common/cmm/getSession.do?"+ new Date().getTime(), {}, function(sesdata){
		...
		if (utype=='30') {
			...
			$(this).jqGridTable("table_list_1","pager_list_1",{
		        ...
		        colNames:["宿舍编号","宿舍管理员","床位数量","所属班级","班级辅导员","水费余额","电费余额","操作"],//"最大审核级别",
		        colModel:[
		          {name:"no",index:"no"},
		          {name:"admin",index:"admin"},
		          {name:"bedqty",index:"bedqty"},
		          {name:"classidNm",index:"classidNm"},
		          {name:"teacher",index:"teacher"},
		          {name:"waterlast",index:"addr"},
		          {name:"spotlast",index:"descr"},
			...
			$(this).jqGridTable("table_list_1","pager_list_1",{
		        url:_this_page_data_url,
		        postData : JSON.parse($(this).form2JsonString("formId1")),
		        colNames:["宿舍编号","宿舍管理员","床位数量","所属班级","班级辅导员","水费余额","电费余额"],//"最大审核级别",
		        colModel:[
		          {name:"no",index:"no"},
		          {name:"admin",index:"admin"},
		          {name:"bedqty",index:"bedqty"},
