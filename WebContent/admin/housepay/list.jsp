    ...
    $.getJSON("../../common/cmm/getSession.do?"+ new Date().getTime(), {}, function(sesdata){
		...
		if (utype=='20') {
			...
			$(this).jqGridTable("table_list_1","pager_list_1",{
		        ...
		        colNames:["宿舍编号","缴费类型","缴费金额","状态"],//"最大审核级别",
		        colModel:[
		          {name:"no",index:"no"},
		          {name:"paytypeNm",index:"paytypeNm"},
		          {name:"amt",index:"amt"},
		    ...
		    $(this).jqGridTable("table_list_1","pager_list_1",{
		        url:_this_page_data_url,
		        postData : JSON.parse($(this).form2JsonString("formId1")),
		        colNames:["宿舍编号","缴费类型","缴费金额","状态","操作"],//"最大审核级别",
		        colModel:[
		          {name:"no",index:"no"},
		          {name:"paytypeNm",index:"paytypeNm"},
		          {name:"amt",index:"amt"},
		          ...
		          {name:"url",index:"url",formatter:function(cellvalue, options, rowObject){
		        	 	...
		        	 	if (rowObject.status=='10') {
			        	 	optstr += '<a class="ml-10" onclick="update('+rowObject.id+')" >确认</a>';
		        	 	} else {
		        	 		optstr = "-";
		        	 	}
		         		return optstr;
